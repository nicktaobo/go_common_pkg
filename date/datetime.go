...
const (
	YmdhmsEmpty = Layout("20060102030405")
	YmdEmpty    = Layout("20060102")
	YmdhmsDash  = Layout("2006-01-02 03:04:05")
	YmdDash     = Layout("2006-01-02")
	YmdhmsSlash = Layout("2006/01/02 03:04:05")
	YmdSlash    = Layout("2006/01/02")
	YmdhmsZh    = Layout("2006年01月02日 03时04分05秒")
...
func AddHour(src time.Time, hour time.Duration) time.Time {
	return src.Add(time.Hour * hour)
}
func SubHour(src time.Time, hour time.Duration) time.Time {
	return src.Add(-time.Hour * hour)
}
